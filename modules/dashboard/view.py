...
class DashboardView(QWidget):
    ...
    def _create_greeting_block(self) -> QWidget:
        ...
        completed_pixmap = QPixmap(str(get_resource_path("resources/icons/task1.png")))
        ...
        worked_icon = QLabel()
        # ✅ ИСПРАВЛЕНО: используем get_resource_path
        worked_pixmap = QPixmap(str(get_resource_path("resources/icons/time1.png")))
    ...
    def _create_active_topic_block(self) -> QWidget:
        ...
        icon_label.setAlignment(Qt.AlignCenter)
        # ✅ ИСПРАВЛЕНО: используем get_resource_path
        pixmap = QPixmap(str(get_resource_path("resources/icons/activ_topic.png")))
    ...
    def _create_last_session_block(self) -> QWidget:
        ...
        icon_label.setAlignment(Qt.AlignCenter)
        # ✅ ИСПРАВЛЕНО: используем get_resource_path
        pixmap = QPixmap(str(get_resource_path("resources/icons/session.png")))
        if not pixmap.isNull():
            pixmap = pixmap.scaled(20, 20, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            icon_label.setPixmap(pixmap)
        else:
        ...
        # 🆕 Время и концентрация — простые QLabel (вместо QWidget с динамическими лейаутами)
        stats_layout = QHBoxLayout()
        stats_layout.setSpacing(16)

        self.last_session_duration = QLabel("⏱️ —")
        self.last_session_duration.setStyleSheet("color: #5A6B7C; font-size: 13px; background-color: transparent;")

        self.last_session_conc = QLabel("🧠 —")
        self.last_session_conc.setStyleSheet("color: #5A6B7C; font-size: 13px; background-color: transparent;")

        self.last_session_energy = QLabel("⚡ —")
        self.last_session_energy.setStyleSheet("color: #5A6B7C; font-size: 13px; background-color: transparent;")

        self.last_session_interest = QLabel("❤️ —")
        self.last_session_interest.setStyleSheet("color: #5A6B7C; font-size: 13px; background-color: transparent;")

        stats_layout.addWidget(self.last_session_duration)
        stats_layout.addWidget(self.last_session_conc)
        stats_layout.addWidget(self.last_session_energy)
        stats_layout.addWidget(self.last_session_interest)
    ...
    def _create_quick_start_block(self) -> QWidget:
        ...
        icon_label = QLabel()
        # ✅ ИСПРАВЛЕНО: используем get_resource_path
        pixmap = QPixmap(str(get_resource_path("resources/icons/rocket.png")))
        if not pixmap.isNull():
            pixmap = pixmap.scaled(20, 20, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            icon_label.setPixmap(pixmap)
        else:
        ...
        # ✅ ИСПРАВЛЕНО: используем get_resource_path
        start_icon = QPixmap(str(get_resource_path("resources/icons/play1.png")))
    ...
    def _create_urgent_tasks_block(self) -> QWidget:
        ...
        icon_label = QLabel()
        # ✅ ИСПРАВЛЕНО: используем get_resource_path
        pixmap = QPixmap(str(get_resource_path("resources/icons/warning.png")))
        if not pixmap.isNull():
            pixmap = pixmap.scaled(20, 20, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            icon_label.setPixmap(pixmap)
        else:
    ...
    def _create_today_analytics_block(self) -> QWidget:
        ...
        icon_label = QLabel()
        # ✅ ИСПРАВЛЕНО: используем get_resource_path
        pixmap = QPixmap(str(get_resource_path("resources/icons/analitics.png")))
        if not pixmap.isNull():
            pixmap = pixmap.scaled(20, 20, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            icon_label.setPixmap(pixmap)
        else:
        ...
        # 🆕 Прогресс-бар для концентрации (правильная реализация через layout)
        conc_progress_container = QWidget()
        conc_progress_container.setFixedHeight(6)
        conc_progress_layout = QHBoxLayout(conc_progress_container)
        conc_progress_layout.setContentsMargins(0, 0, 0, 0)
        conc_progress_layout.setSpacing(0)

        self.conc_fill = QFrame()
        ...
        """)
        self.conc_fill.setMinimumWidth(0)

        conc_progress_bg = QFrame()
        conc_progress_bg.setStyleSheet("""
            QFrame {
        ...
        conc_progress_layout.addWidget(self.conc_fill, 0)
        conc_progress_layout.addWidget(conc_progress_bg, 1)
        conc_layout.addWidget(conc_progress_container)
        ...
        #  Прогресс-бар для энергии (правильная реализация через layout)
        energy_progress_container = QWidget()
        energy_progress_container.setFixedHeight(6)
        energy_progress_layout = QHBoxLayout(energy_progress_container)
        energy_progress_layout.setContentsMargins(0, 0, 0, 0)
        energy_progress_layout.setSpacing(0)

        self.energy_fill = QFrame()
        ...
        """)
        self.energy_fill.setMinimumWidth(0)

        energy_progress_bg = QFrame()
        energy_progress_bg.setStyleSheet("""
            QFrame {
                background-color: #E5E7EB;
                border-radius: 3px;
                border: none;
        ...
        energy_progress_layout.addWidget(self.energy_fill, 0)
        energy_progress_layout.addWidget(energy_progress_bg, 1)
        energy_layout.addWidget(energy_progress_container)

        stats_layout.addWidget(energy_card, 1)

        # --- 🆕 Карточка интереса с прогресс-баром ---
        interest_card = QFrame()
        interest_card.setStyleSheet("""
                    QFrame {
                        background-color: #F9FAFB;
                        border-radius: 12px;
                        border: none;
                ...
                """)
        interest_layout = QVBoxLayout(interest_card)
        interest_layout.setContentsMargins(12, 10, 12, 10)
        interest_layout.setSpacing(4)

        self.today_interest_label = QLabel("0")
        self.today_interest_label.setStyleSheet(
            "font-size: 22px; font-weight: bold; color: #1F2937; background-color: transparent;")
        interest_layout.addWidget(self.today_interest_label)

        self.today_interest_desc = QLabel("интерес")
        self.today_interest_desc.setStyleSheet("color: #6B7280; font-size: 12px; background-color: transparent;")
        interest_layout.addWidget(self.today_interest_desc)

        # Прогресс-бар для интереса (розовый цвет)
        interest_progress_container = QWidget()
        interest_progress_container.setFixedHeight(6)
        interest_progress_layout = QHBoxLayout(interest_progress_container)
        interest_progress_layout.setContentsMargins(0, 0, 0, 0)
        interest_progress_layout.setSpacing(0)

        self.interest_fill = QFrame()
        self.interest_fill.setStyleSheet("""
                    QFrame {
                ...
                """)
        self.interest_fill.setMinimumWidth(0)

        interest_progress_bg = QFrame()
        interest_progress_bg.setStyleSheet("""
                    QFrame {
                        background-color: #E5E7EB;
                        border-radius: 3px;
                        border: none;
        ...
        interest_progress_layout.addWidget(self.interest_fill, 0)
        interest_progress_layout.addWidget(interest_progress_bg, 1)
        interest_layout.addWidget(interest_progress_container)

        stats_layout.addWidget(interest_card, 1)  # Добавляем в общий ряд
    ...
    def resizeEvent(self, event):
        """Пересчитываем прогресс-бары при изменении размера окна"""
        super().resizeEvent(event)
        # Пересчитываем на основе текущих значений
        try:
            conc_text = self.today_conc_label.text()
            energy_text = self.today_energy_label.text()
            interest_text = self.today_interest_label.text()
            conc_val = int(conc_text) if conc_text.isdigit() else 0
            energy_val = int(energy_text) if energy_text.isdigit() else 0
            interest_val = int(interest_text) if interest_text.isdigit() else 0
    ...
    def refresh(self):
        ...
        try:
            ...
            self.kpi_row.add_card("Заметки", str(total_stats['total_notes']),
                                  str(get_resource_path("resources/icons/notes1.png")))
            self.kpi_row.add_card("Карточки", str(total_stats['total_flashcards']),
                                  str(get_resource_path("resources/icons/flashcard1.png")))
            ...
            if last_session is not None:
                ...
                self.last_session_energy.setText(f"⚡ {last_session['avg_energy']}/100")
            else:
            ...
            logger.error(f"Критическая ошибка в refresh(): {e}", exc_info=True)
