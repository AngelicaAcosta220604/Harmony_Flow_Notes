...
class TopicSessionCard(QFrame):
    ...
    def __init__(self, session_data: dict, parent=None):
        ...
        self._setup_ui()
        # ✅ ИСПРАВЛЕНО: УБРАНА подписка на event_bus из этого класса!
        # Подписка должна быть в TopicView, а не в карточке сессии
    ...
    def _setup_ui(self):
        try:
            ...
            logger.error(f"Ошибка настройки TopicSessionCard: {e}", exc_info=True)
...
class NoteListItemWidget(QWidget):
    ...
    def _setup_ui(self, title: str, date_str: str):
        try:
            ...
            logger.error(f"Ошибка настройки NoteListItemWidget: {e}", exc_info=True)

    # ✅ ИСПРАВЛЕНО: УБРАНЫ методы _on_note_changed, _on_flashcard_changed, _on_task_changed
    # Они должны быть в TopicView, а не здесь!
...
class TaskListItemWidget(QWidget):
    ...
    def _setup_ui(self, title: str, deadline: str, status: str, is_overdue: bool):
        try:
            ...
            logger.error(f"Ошибка настройки TaskListItemWidget: {e}", exc_info=True)
...
class FlashcardListItemWidget(QWidget):
    ...
    def _setup_ui(self, preview: str, card_type: str, date_str: str):
        try:
            ...
            logger.error(f"Ошибка настройки FlashcardListItemWidget: {e}", exc_info=True)
...
class FlashcardViewDialog(QDialog):
    ...
    def _setup_ui(self):
        try:
            ...
            logger.error(f"Ошибка настройки FlashcardViewDialog: {e}", exc_info=True)
...
class TopicView(QWidget):
    ...
    def __init__(
        ...
        # ✅ ИСПРАВЛЕНО: Подписываемся на ВСЕ события для автообновления
        ...
        event_bus.task_updated.connect(self._on_task_changed)

        # ✅ ИСПРАВЛЕНО: ДОБАВЛЕНА подписка на заметки
        event_bus.note_created.connect(self._on_note_changed)
        event_bus.note_updated.connect(self._on_note_changed)
        event_bus.note_deleted.connect(self._on_note_changed)

        # ✅ ИСПРАВЛЕНО: ДОБАВЛЕНА подписка на карточки
        event_bus.flashcard_created.connect(self._on_flashcard_changed)
        event_bus.flashcard_deleted.connect(self._on_flashcard_changed)

    # ✅ ИСПРАВЛЕНО: Методы теперь в правильном классе - TopicView
    ...
    def _on_note_changed(self, note_id: int = None):
        """Обновляет список заметок при изменении"""
        try:
            if self._current_topic_id:
    ...
    def _on_flashcard_changed(self, card_id: int = None):
        """Обновляет список карточек при изменении"""
        try:
            if self._current_topic_id:
    ...
    def _setup_ui(self):
        try:
            ...
            logger.error(f"Ошибка настройки TopicView: {e}", exc_info=True)
    ...
    def _create_overview_tab(self) -> QWidget:
        try:
            ...
            logger.error(f"Ошибка создания overview_tab: {e}", exc_info=True)
            return QWidget()
    ...
    def _create_stat_card(self, icon_path: str, title: str, value: str, color: str = "#3B82F6") -> QFrame:
        try:
            ...
            logger.error(f"Ошибка создания stat_card '{title}': {e}", exc_info=True)
            return QFrame()
    ...
    def _load_topic_sessions(self, topic_id: int):
        ...
        try:
            ...
            sessions.sort(key=lambda s: 0 if s.status == 'active' else (1 if s.status == 'paused' else 2))
