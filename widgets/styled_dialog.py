...
class StyledInputDialog(QDialog):
    def __init__(self, title: str, label: str, parent=None, default_text: str = ""):
        ...
        try:
            ...
            logger.debug(f"StyledInputDialog создан: '{title}'")
        except Exception as e:
            logger.error(f"Ошибка создания StyledInputDialog: {e}", exc_info=True)
    ...
    def _on_ok(self):
        try:
            ...
            if self._result_text:
                ...
                # ✅ ИСПРАВЛЕНО: используем lambda с проверкой
                QTimer.singleShot(500, lambda: self._safe_reset_style())
        except Exception as e:
            logger.error(f"Ошибка в _on_ok: {e}", exc_info=True)

    def _safe_reset_style(self):
        """Безопасный сброс стиля с проверкой существования виджета"""
        try:
            # Проверяем, не удален ли виджет
            if self and self.input and not self.input.isDeleted() if hasattr(self.input, 'isDeleted') else True:
                self._reset_style()
        except RuntimeError as e:
            logger.debug(f"Виджет уже удален, пропускаем сброс стиля: {e}")
        except Exception as e:
            logger.error(f"Ошибка сброса стиля: {e}", exc_info=True)
    ...
    def _reset_style(self):
        try:
            ...
            logger.error(f"Ошибка сброса стиля input: {e}", exc_info=True)
    ...
    def mousePressEvent(self, event):
        try:
            ...
            logger.error(f"Ошибка в mousePressEvent: {e}", exc_info=True)
    ...
    def mouseMoveEvent(self, event):
        try:
            ...
            logger.error(f"Ошибка в mouseMoveEvent: {e}", exc_info=True)
    ...
    def mouseReleaseEvent(self, event):
        try:
            ...
            logger.error(f"Ошибка в mouseReleaseEvent: {e}", exc_info=True)
    ...
    def getText(parent, title: str, label: str, text: str = "") -> tuple:
        try:
            ...
            logger.error(f"Ошибка в getText: {e}", exc_info=True)
            return "", False